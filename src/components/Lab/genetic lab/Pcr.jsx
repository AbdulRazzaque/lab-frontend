...
const Pcr = (props) => {
  ...
  const updateRow = async () => {
    ...
    try {
      ...
      await axios
        .put(
          `${process.env.REACT_APP_DEVELOPMENT}/api/updatePrc/${update._id}`,
          combinedObj,
          { headers: { token: `${accessToken}` } }
        )
        .then((response) => {
      ...
      readXlsxFile(e.target.files[0]).then((rows) => {
        ...
        rows.map((item, id) => {
          if (id !== 0) {
            ...
            axios
              .post(`${process.env.REACT_APP_DEVELOPMENT}/api/addPrc`, obj, {
                headers: { token: `${accessToken}` },
              })
              .then((response) => {
                console.log("Response", response);
  ...
  return (
    <div>
      ...
      <h1 className="text-center my-3 mb-5 heading">Pcr Lab </h1>
